	...
	function main($id, $mode)
	{
		...
		if ($submit)
		{
			...
			if (!sizeof($error))
			{
				...
				// START Mod MadNad Users
				$m_n_row	= array();
				$n_row		= array();
				$only_m_n	= false;
				// END Mod MadNav Users
				...
				{
					// START Mod MadNad Users
					if($group_id && in_array($group_id, array(10, 11, 12, 13), true))
					{
						include $phpbb_root_path . '../madridnavega.org/include/email.php';
						switch ($group_id) {
							case 10 :
								$group_id = $m_n_group_id = 0;
								break;
							case 11 :
								$group_id = $m_n_group_id = 2;
								break;
							case 12 :
								$m_n_group_id = 0;
								$only_m_n = true;
						...
						$mail_banned_flag = isset($_REQUEST['mail_banned_flag']);
						$m_n_row = get_email_row($m_n_group_id, $mail_banned_flag);
...
//						exit();
				...
				$rows = array_merge((array) $n_row, (array) $m_n_row);
...
//				exit;
		...
		$select_list .= '<option value="10"' . (($group_id == 10) ? ' selected="selected"' : '') . '>' . $user->lang['ALL_PLUS_MAD_NAV_USERS'] . '</option>';
		$select_list .= '<option value="11"' . (($group_id == 11) ? ' selected="selected"' : '') . '>' . $user->lang['REGISTERED_PLUS_MAD_NAV'] . '</option>';
		$select_list .= '<option value="12"' . (($group_id == 12) ? ' selected="selected"' : '') . '>' . $user->lang['ALL_USERS_MAD_NAV'] . '</option>';
		$select_list .= '<option value="13"' . (($group_id == 13) ? ' selected="selected"' : '') . '>' . $user->lang['REGISTERED_MAD_NAV'] . '</option>';
