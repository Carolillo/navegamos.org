...
if ($config['presentations_view_enable'] && !$auth->acl_get('u_presentations_view'))
{
	...
	if($user_publicated == 0 && $forum_id != $presentation_forum)
	{
		...
		trigger_error($user->lang['PRESENTATIONS_NO_READ'] . '<br /><br />' . sprintf($user->lang['PRESENTATIONS_FORUM'], '<a href="' . append_sid("{$phpbb_root_path}posting.$phpEx", "mode=post&f=" . $presentation_forum) . '">', '</a>'));
	}
