...
// START Presentations obligated mod
$user->setup('mods/presentations');
// END Presentations obligated mod
...
// START Presentations obligated mod
$presentation_forum = $config['presentations_forum'];
if ($config['presentations_post_enable'] && !$auth->acl_get('u_presentations_post'))
{
	$sql = 'SELECT topic_poster
			FROM ' . TOPICS_TABLE . '
			WHERE topic_poster = ' . $user->data['user_id'];
	$result = $db->sql_query($sql);
	$user_publicated = $db->sql_fetchrow($result);
	if($user_publicated == 0 && $forum_id != $presentation_forum)
	{
		trigger_error($user->lang['PRESENTATIONS_NO_POST'] . '<br /><br />' . sprintf($user->lang['PRESENTATIONS_FORUM'], '<a href="' . append_sid("{$phpbb_root_path}posting.$phpEx", "mode=post&f=" . $presentation_forum) . '">', '</a>'));
	}
...
if ($submit || $preview || $refresh)
{
	...
	// START Presentations obligated mod
	if (($config['presentations_form_enable'] == true) && $config['presentations_post_enable'] && ($forum_id == $presentation_forum) && ($mode == 'post'))
	{
		$usr_name 				= utf8_normalize_nfc(request_var('usr_name', '', true ));
		$usr_city 				=utf8_normalize_nfc(request_var('usr_city', '', true ));
		$usr_country 			= utf8_normalize_nfc(request_var('usr_country', '', true ));
		$usr_virtues 			= utf8_normalize_nfc(request_var('usr_virtues', '', true ));
		$usr_defects 			= utf8_normalize_nfc(request_var('usr_defects', '', true ));
		$usr_bdate 				= utf8_normalize_nfc(request_var('usr_bdate', '', true ));
		$usr_hobbies 			= utf8_normalize_nfc(request_var('usr_hobbies', '', true ));
		$message				= utf8_normalize_nfc(request_var('message', '', true));
		$presentations_message 	= utf8_normalize_nfc(request_var('message', '', true));
	
		$messagetemp = '';
	
		$u1 = $user->lang['USR_NAME'];
		$u2 = $user->lang['USR_CITY'];
		$u3 = $user->lang['USR_COUNTRY'];
		$u4 = $user->lang['USR_BDATE'];
		$u5 = $user->lang['USR_VIRTUES'];
		$u6 = $user->lang['USR_DEFECTS'];
		$u7 = $user->lang['USR_HOBBIES'];
		$u0 = $user->lang['USR_HEAD_MSG'];
	
		$messagetemp .= "[b]" . $u1 . "[/b] " . $usr_name . "\n";
		$messagetemp .= "[b]" . $u2 . "[/b] " . $usr_city . "\n";
		$messagetemp .= "[b]" . $u3 . "[/b] " . $usr_country . "\n";
		$messagetemp .= "[b]" . $u4 . "[/b] " . $usr_bdate . "\n";
		$messagetemp .= "[b]" . $u5 . "[/b] " . $usr_virtues . "\n";
		$messagetemp .= "[b]" . $u6 . "[/b] " . $usr_defects . "\n";
		$messagetemp .= "[b]" . $u7 . "[/b] " . $usr_hobbies . "\n";
		$messagetemp .= "\n";
		$messagetemp .= "[b]" . $u0 . "[/b]\n[i]" . $message . "[/i]";
...
//	'MESSAGE'				=> $post_data['post_text'],
	...
	// START Presentations obligated mod
	'MESSAGE'				=> (isset($presentations_message)) ? $presentations_message : $post_data['post_text'],
	// END Presentations obligated mod
	...
	// START Presentations obligated mod
	'PRESENTATIONS_FORM_ACTIVE' => (($config['presentations_form_enable'] == true) && $config['presentations_post_enable'] && ($forum_id == $presentation_forum) && ($mode == 'post')) ? true : false,
	'USR_NAME'					=> request_var('usr_name', '', true),
	'USR_CITY'					=> request_var('usr_city', '', true),
	'USR_COUNTRY'				=> request_var('usr_country', '', true),
	'USR_BDATE'					=> request_var('usr_bdate', '', true),
	'USR_VIRTUES'				=> request_var('usr_virtues', '', true),
	'USR_DEFECTS'				=> request_var('usr_defects', '', true),
	'USR_HOBBIES'				=> request_var('usr_hobbies', '', true),
	// END Presentations obligated mod
